...
- **后端 API**：`ViLa-MIL/api_server.py`，训练子进程调用 `main_LUSC.py`；支持多种 MIL 与决策级集成 `EnsembleDecision`。
- **前端**：`vila-mil-frontend/`，构建后由 Nginx 托管；请求前缀为 **`/api`**（与网关反代一致）。
- **可选 TRIDENT**：Docker 中通过卷挂载 `TRIDENT/` 与相关环境变量，用于 WSI 特征提取（详见 `docker-compose.yml`）。

### 当前版本重点（2026-04）

- **集成方法主线**：`EnsembleDecision` 作为决策级集成模型，当前固定 `avg_prob`（概率融合）策略。
- **集成过程可视化**：无逐 epoch 训练时，后端会生成 fold 级过程点并扩展为统一 480 点展示，便于与单模型同图对比。
- **Clinical 入口简化**：`已有 H5 文件` + `从 WSI 生成` 两种方式；后者保留 `快速预览` 与 `正式预测`。
- **真快速模式**：WSI 在 `quick=true` 时走低采样近似路径；`formal` 走 TRIDENT 全量路径。
- **WSI 扩展名支持**：`.svs/.ndpi/.mrxs/.scn/.tif/.tiff` 已打通上传与关联流程。
...
- 当前可选模型标识（与前端下拉一致）包括但不限于：`ViLa_MIL`、`TransMIL`、`AMIL`、`WiKG`、`RRTMIL`、`PatchGCN`、`surformer`、`DSMIL`、`S4MIL`、`EnsembleDecision`（以后端 `MODEL_CHOICES` 为准）。

决策级集成：五路基线各自输出后再固定概率融合，见 `ViLa-MIL/models/EnsembleDecision.py` 与 `main_LUSC.py --decision_fusion`（当前仅 `avg_prob`）。

### Clinical 的 WSI 生成模式

- **快速预览**：低采样近似流程（速度优先，适合联调）。
- **正式预测**：TRIDENT 全量特征提取（结果优先，耗时更长）。
- 前端提示文案与后端执行逻辑已对齐，避免“模式名不同但走同一路径”的误解。

### 常见问题（FAQ）

1. **为什么 EnsembleDecision 曲线点少？**  
   原始日志是 fold 级评估点。为统一展示，后端会补齐到 480 点并做显示层平滑，不改变原始指标。

2. **为什么 GitHub push 很慢/超时？**  
   常见原因是 HTTPS 443 网络不通或仓库包含大文件（如 `.svs`）。建议改用 SSH 远程，且对大文件使用 Git LFS。

3. **WSI 上传报扩展名不支持？**  
   请确认文件后缀在支持列表内，并更新到最新代码版本；当前接口已支持主流 WSI 后缀。
