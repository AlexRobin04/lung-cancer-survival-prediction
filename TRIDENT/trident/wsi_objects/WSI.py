...
class WSI:
    ...
    def _segment_semantic(
        ...
        if self.max_workers == 0:
            ...
            effective_num_workers = (
                get_num_workers(batch_size, max_workers=self.max_workers) if num_workers is None else num_workers
            )
    ...
    def extract_patch_features(
        ...
        feat_num_workers = 0 if self.max_workers == 0 else get_num_workers(batch_limit, max_workers=self.max_workers)
        dataloader = DataLoader(dataset, batch_size=batch_limit, num_workers=feat_num_workers, pin_memory=False)
