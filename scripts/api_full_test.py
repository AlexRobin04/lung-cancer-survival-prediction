...
def main() -> int:
    ...
    cases: list[Case] = [
        ...
        Case("eval km from predictions no taskId", "GET", "/api/evaluation/km/from-predictions", 400),
        Case(
            "eval km from predictions",
            "GET",
            f"/api/evaluation/km/from-predictions?taskId={fake_tid}",
            (200, 500),
            note="200 时 ok 可能为 false；500 若未装 lifelines",
        ),
        Case("eval km six best by model", "GET", "/api/evaluation/km/six-best-by-model", (200, 500), note="500 若未装 lifelines"),
