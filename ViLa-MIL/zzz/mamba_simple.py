...
from pscan import pscan, pscan_2d
...
class MambaBlock(nn.Module):
    def __init__(self, config: MambaConfig):
        ...
        if self.config.use_cuda:
            try:
                if self.config.mamba_2d:
                    from models.pscan_2d import selective_scan_fn as selective_scan_fn
    ...
    def ssm_2d(self, x, z=None):
        ...
        if self.config.use_cuda:
            ...
            y = self.selective_scan_cuda(x, delta, A, B, C, D, z=z, delta_softplus=True, delta_bias=self.dt_proj.bias.float(), HH=self.config.HH, WW=self.config.WW)
            y = y.transpose(1, 2)
    ...
    def selective_scan_seq_2d(self, x, delta, A, B, C, D):
        ...
        for t in range(0, L):
            ...
            if col_id == 0:
                ...
                h = deltaA[:, t] * hs[t-image_shape] + BX[:, t] # h = A*(state of left patch)  + B*(input of current patch)
            else:
...
def reconstruct_2d_wsi(feats, coords, max_w, max_h, patch_size, pad_token):
    ...
    """
    normalized_coords = coords // patch_size
    w_indexes = normalized_coords[:, 0].int()
    h_indexes = normalized_coords[:, 1].int()
    
    max_w = max_w // patch_size + 1
    max_h = max_h // patch_size + 1
    
    d = feats.shape[-1]
    
    results = pad_token.expand((1, max_h, max_w, d)).to(feats.device).clone()
    results[:, h_indexes, w_indexes, :] = feats
