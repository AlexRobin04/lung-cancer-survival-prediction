...
def collate_tranformer(batch):
	"""
	Transformer mode collate.

	当前训练循环（utils/core_utils.py）按 5 元组解包：
	(data_s, coord_s, data_l, coords_l, label)

	历史版本可能曾返回更多字段（status/time/...）。这里做兼容：
	- batch item 长度为 5：返回 5 项
	- batch item 长度 >= 12：返回前 12 项的旧格式
	"""
	...
	# New/expected path: 5-tuple
	if len(batch[0]) == 5:
		return [img_s, coord_s, img_l, coord_l, label]

	# Backward-compatible path: 12-tuple
	status = torch.LongTensor([item[5] for item in batch])
	time = torch.FloatTensor([item[6] for item in batch])
	disc = torch.LongTensor([item[7] for item in batch])
	soft_0 = torch.FloatTensor([item[8] for item in batch])
	soft_1 = torch.FloatTensor([item[9] for item in batch])
	soft_2 = torch.FloatTensor([item[10] for item in batch])
	soft_3 = torch.FloatTensor([item[11] for item in batch])

	return [img_s, coord_s, img_l, coord_l, label, status, time, disc, soft_0, soft_1, soft_2, soft_3]


def get_simple_loader(dataset, batch_size=1, num_workers=64, mode='clam'):
	if(mode == 'transformer'):
		collate = collate_tranformer

	kwargs = {'num_workers': 64, 'pin_memory': False, 'num_workers': num_workers} if device.type == "cuda" else {}
...
def get_split_loader(split_dataset, training = False, testing = False, weighted = False, mode='clam'):
	...
	kwargs = {'num_workers': 64} if device.type == "cuda" else {}
	if not testing:
		if training:
			if weighted:
...
def print_network(net):
	...
	try:
		...
		print(f"{net.__class__.__name__}(repr_failed: {e})")
